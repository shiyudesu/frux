# adaptive-web-playback Specification
...
- **WHEN** the same generation URL is requested again while its redirect and media response remain fresh
...
- **THEN** the player treats it as a new source revision and does not reuse stale media state from the previous generation
...
- **THEN** the player receives `private, no-store` behavior and does not persist it as public media cache

#### Scenario: Browser does not reuse Range cache
- **WHEN** a browser chooses not to reuse cached partial video responses
- **THEN** playback remains correct and falls back to a normal bounded signed request without claiming a guaranteed cache hit
