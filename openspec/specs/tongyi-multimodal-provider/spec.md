# tongyi-multimodal-provider Specification

## Purpose

Defines the supported Alibaba Cloud Tongyi Embedding Vision Flash profiles, their Frux contract
identities, provider-specific request translation, validation, startup probing, and bounded
observability.
...
When the dated snapshot profile is selected, the adapter SHALL translate a valid Frux video request
into one DashScope input content object whose `text` is the canonical public video text and whose
`multi_images` are ordered Base64 Data URIs for the prepared cover/keyframes. It SHALL request dense
dimension 768 and resolution level 1 and SHALL NOT send original video URLs, object keys, credentials,
user identity, or behavior metadata.
...
exact valid finite non-zero upstream vector set, normalize the result, calculate its digest, and never
expose the API key, raw upstream body, endpoint, or arbitrary upstream error through the Frux protocol.
