...
The narrowed `project-semantic-user-interest` change creates a live-only shadow projection keyed by `(user_id, model)`. It defines `semantic-interest-v1`, exact-model video-vector validation, eligible behavior/action/feedback facts, weights, event-time decay, model-scoped applied-event identity, and a stable `(user_id, model)` advisory-lock namespace. It deliberately leaves historical facts, disabled intervals, and completeness repair to this change.
...
Historical reconstruction must tolerate interruption, late and out-of-order source facts, incomplete video-vector coverage, and concurrent live projection. It must never infer embeddings, invent author content, publish a silently partial profile, mark a missing-vector fact applied, or replace live state without preserving newer live contributions.
...
- Reuse the live projector's eligibility, classification, weights, canonical identity, ordering, event-time decay, clamping, schema, and vector validation.
...
- Defer users with missing or invalid exact-model embeddings without changing their profile or applied-event ledger.
...
where source-kind rank is fixed by `semantic-interest-v1`. Fact pages use the same tuple as an opaque internal cursor. The normalizer invokes the shared live eligibility and payload-hash code, so unsupported, inactive, malformed, below-threshold, and author-only facts do not contribute.
...
### 4. Reconstruct only complete users and treat missing vectors as durable deferrals

The runner reads exact-model video embeddings in bounded batches for all contributing facts, deduplicated by video ID. Every source vector must match the selected model, dimension, finiteness, and normalization contract. The runner uses the shared semantic reducer to compute long-term, recent, and negative vectors from zero in canonical fact order.

If any contributing fact lacks a valid exact-model embedding, exceeds the per-user event bound, or cannot be safely normalized, the user is durably deferred with bounded reason `missing_embedding`, `invalid_embedding`, `event_limit`, or `invalid_fact`. No replacement profile, coverage row, or new applied-event row is written for that user. Exact missing identities are retained only in bounded operational state sufficient to retry; vectors and source payloads are not copied.
...
### 5. Share the semantic reducer rather than duplicate projection rules
...
- completion precedence, destination vectors, fixed weights, and payload hashes;
- the fixed source ordering and tie-breaker;
- 30-day long-term and 24-hour recent/negative half-lives;
- delayed contribution decay, monotonic materialization, component bounds, and dimension checks.

The rebuild materialization time is the maximum occurrence time among included facts, or the existing schema-defined zero state when there are no contributing facts. Processing time, scan page, retry attempt, and restart time never enter the vectors. Tests compare reconstruction with live application of the same facts in canonical, delayed, and shuffled delivery orders.

Alternative considered: implement a SQL aggregate. Rejected because separate arithmetic would drift from live decay, clamping, and future schema-version behavior.
...
2. locks the current semantic profile and reads its current version plus exact-model applied ledgers;
3. captures current per-source maxima as a user catch-up fence;
4. loads all eligible user facts after the run fence through that catch-up fence, plus resolves every existing exact-model ledger identity not already represented by the baseline;
...
6. locks all referenced exact-model embedding rows in stable video-ID order and verifies that the vector digests used outside the transaction remain current;
7. recomputes the complete profile from zero with baseline plus catch-up facts in canonical order;
8. conditionally replaces/upserts the profile at `locked_current_version + 1`, upserts matching applied-event rows, records rebuild coverage, clears the user's deferral, and commits atomically.
...
### 7. Make replacement, ledger population, and coverage idempotent

The transaction validates existing ledger payload hashes before writing. It inserts absent semantic applied-event rows for every included fact and treats matching rows as duplicates; a mismatched identity is a terminal user conflict. It never deletes applied-event rows.
...
The binary is added to the existing API image with a manual Compose profile/entrypoint and no automatic service startup.
...
- [Missing video coverage prevents profile publication] → Report exact-model coverage safely, retain a durable deferral, and use `backfill-semantic-video-embeddings` before restart.
...
1. Complete and strictly validate `integrate-semantic-video-embeddings`, `backfill-semantic-video-embeddings`, and the narrowed `project-semantic-user-interest`.
...
5. Build the binary into the API image and add a manual Compose profile plus operator documentation.
...
Rollback stops the one-shot command and disables its manual entrypoint. Live projection continues unchanged. Successfully committed profiles and applied-event rows remain valid; active run state remains resumable. Rebuild-owned operational rows may be cleaned later only after confirming no restart is required.

## Open Questions

None. Dependencies, exact model/schema, source fences, ordering, decay reuse, missing-vector behavior, live-race protocol, force guard, bounds, checkpoint semantics, observability, and exclusions are fixed by this proposal.
