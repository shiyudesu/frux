...
- Build an additive, exact-model, rebuildable `vector(384)` projection from authoritative JSONB embedding rows.
...
### 2. Extend the existing advisory-locked migration only in enabled mode
...
4. creates the additive projection table, constraints, and supporting B-tree indexes;
5. creates the model-specific cosine HNSW index;
6. records the migration marker only after all validation succeeds.
...
The HNSW index is partial and exact-model isolated:
...
Supporting indexes cover `(model, source_embedding_updated_at, video_id)` on the projection, `(model, updated_at, video_id)` on `video_embedding`, and readable-video reconciliation predicates. Names are schema-unique. No existing column/table is rewritten and no source embedding is deleted.
...
A projection row is current only when its model, source text hash, and source embedding `updated_at` match the durable row and the video is currently:
...
Queries always rejoin and recheck the live video row, so a projection lag cannot expose an unreadable video. The reconciler deletes projection rows whose source disappeared, model no longer matches, or video became private, unpublished, deleted, or media-unready. Hard-deleted videos are also removed by the foreign-key cascade.
...
Each cycle takes a projection-specific PostgreSQL advisory lock with `pg_try_advisory_lock`; another process holding it causes a healthy skipped cycle. It first selects bounded source rows that are missing or differ by text hash/source update time, ordered by `(video_embedding.updated_at, video_id)`, then validates and upserts them with `ON CONFLICT (video_id, model) DO UPDATE` only when source metadata differs. It next selects bounded stale/ineligible projection IDs ordered by `(source_embedding_updated_at, video_id)` and deletes only those exact model rows.

Transactions are per batch, context-bound, and short. Repeated cycles, concurrent live embedding writes, concurrent historical backfill writes, crashes, and duplicate selection are safe. Invalid source JSON is never projected; it is counted as invalid and remains visible as missing coverage for remediation by the source-owning changes.
...
### 5. Add a guarded one-shot operator command for dry-run, catch-up, purge, and rebuild
...
- `--rebuild-model`, which purges that exact projection then performs bounded reconciliation;
- `--reindex`, which runs a guarded concurrent rebuild of the named model-specific HNSW index.

All mutating destructive modes require `--confirm-model=semantic-minilm-l12-v2@e8f8c211226b894f`. Reindex additionally requires `--confirm-index=idx_semantic_video_ann_minilm_cosine_hnsw`. Page size, maximum rows, and maximum runtime are positive and bounded; zero/unlimited values are rejected. The command uses the same advisory lock so it cannot overlap a worker cycle or another operator mutation.
...
### 6. Expose a fixed-model infrastructure ANN repository with plain values
...
### 7. Make coverage and operations observable with bounded labels
...
### 8. Verify correctness and usefulness against real PostgreSQL
...
- `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)` on at least 10,000 deterministic vectors after `ANALYZE`, requiring the named HNSW index and rejecting a sequential scan of the projection;
- ANN recall against exact cosine ground truth on the same seeded fixture, requiring recall@20 of at least `0.90` across at least 100 deterministic queries;
...
### 9. Roll out and roll back without changing recommendation behavior
...
4. enable migration and verify extension/schema/index prerequisites;
5. run dry-run, then bounded reconciliation until coverage is acceptable;
6. run plan, recall, and modest performance acceptance;
...
- [HNSW build or rebuild consumes database resources] → Use explicit modest parameters, guarded operator execution, concurrent reindex where supported, and documented maintenance windows.
...
3. Add advisory-locked extension/projection/index migration and real PostgreSQL migration tests.
...
6. Run seeded query-plan, recall-quality, and modest performance gates.
