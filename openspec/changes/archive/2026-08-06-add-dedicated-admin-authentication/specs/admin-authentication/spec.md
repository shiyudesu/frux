...
Frux SHALL issue admin credentials with a distinct `admin_access` token type, `frux-admin` audience, bounded expiration, and account subject, and SHALL require that purpose on protected admin routes.

#### Scenario: Admin token accesses an admin route
- **WHEN** a valid unexpired `admin_access` token with the admin audience is supplied to a protected admin route
