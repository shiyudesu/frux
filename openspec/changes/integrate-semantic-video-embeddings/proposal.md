...
- Make video-published processing persist hash coverage first, then attempt semantic generation with durable delayed RabbitMQ retries, idempotent `(video_id, model)` writes, and isolation from unrelated worker consumers.
...
- Add bounded-cardinality metrics for semantic requests, live-event vector outcomes and retries, semantic coverage, and queue backlog.
...
- Affects the Go embedding domain/application packages, RabbitMQ embedding consumer topology, PostgreSQL embedding repository, worker composition, configuration, metrics, Compose wiring, tests, and embedding/video operational documentation.
