...
func (r *humanReviewAPIMemoryRepo) ListHumanQueue(_ context.Context, filter domainreview.HumanQueueFilter) ([]*domainreview.HumanQueueItem, error) {
	...
	for _, reviewCase := range r.cases {
		...
		if filter.Cursor != nil && !(reviewCase.Priority < filter.Cursor.Priority ||
			(reviewCase.Priority == filter.Cursor.Priority && reviewCase.CreatedAt.After(filter.Cursor.CreatedAt)) ||
			(reviewCase.Priority == filter.Cursor.Priority && reviewCase.CreatedAt.Equal(filter.Cursor.CreatedAt) && reviewCase.ID > filter.Cursor.CaseID)) {
			continue
		}
		copyCase := *reviewCase
...
func (r *humanReviewAPIMemoryRepo) GetHumanCaseDetail(_ context.Context, caseID int64) (*domainreview.HumanCaseDetail, error) {
	...
	return &domainreview.HumanCaseDetail{
		Case:    &copyCase,
		Subject: domainreview.ReviewSubject{VideoID: reviewCase.VideoID, AuthorID: 9, Title: "video"},
	}, nil
}
...
func newHumanReviewAPIRouter(repo *humanReviewAPIMemoryRepo, principal *domainaccount.AdminPrincipal) *server.Hertz {
	service := applicationreview.New(
		...
		applicationreview.WithHumanTokenReader(bytes.NewReader(bytes.Repeat([]byte{5}, 128))),
	)
...
func TestHumanReviewAdminAPIFlow(t *testing.T) {
	...
	if len(page.Items) != 2 || page.Items[0].Case.ID != 1 || page.NextCursor == "" {
	...
	if lease.LeaseToken == "" || lease.Case.Version != 2 {
	...
	renewBody := fmt.Sprintf(`{"lease_token":%q,"expected_case_version":2}`, lease.LeaseToken)
	renewed := performHumanReviewJSON(
		t, router, http.MethodPost, "/api/admin/review/cases/1/lease/renew", renewBody, "",
	)
	if renewed.Code != http.StatusOK || !strings.Contains(renewed.Body.String(), `"version":3`) {
		t.Fatalf("renew status=%d body=%s", renewed.Code, renewed.Body.String())
	}
	releaseBody := fmt.Sprintf(`{"lease_token":%q,"expected_case_version":3}`, lease.LeaseToken)
	released := performHumanReviewJSON(
		t, router, http.MethodDelete, "/api/admin/review/cases/1/lease", releaseBody, "",
	)
	if released.Code != http.StatusOK || !strings.Contains(released.Body.String(), `"version":4`) {
	...
	decisionBody := fmt.Sprintf(
		`{"lease_token":%q,"expected_case_version":5,"review_version":1,"outcome":"approve","reason_code":"content_compliant","note":""}`,
		lease.LeaseToken,
	)
