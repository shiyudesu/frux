...
func TestWriteUploadSessionErrorDistinguishesFileConstraints(t *testing.T) {
	tests := []struct {
		name string
		err  error
		code string
		...
		{
			name: "file type",
			err:  applicationmedia.ErrUploadFileTypeInvalid,
			code: interfaceshttpapierror.CodeUploadFileTypeInvalid,
		},
		{
			name: "video size",
			err:  applicationmedia.ErrVideoUploadTooLarge,
			code: interfaceshttpapierror.CodeUploadVideoTooLarge,
		},
		{
			name: "cover size",
			err:  applicationmedia.ErrCoverUploadTooLarge,
