...
type FeedCache struct {
	...
	statSource      FeedStatSource
}

type FeedCacheOption func(*FeedCache)

// FeedStatSource supplies authoritative, versioned PostgreSQL snapshots when
// Redis has lost its revision fence. Never re-seed from an in-flight old value.
type FeedStatSource interface {
	BatchGetFeedStats(context.Context, []int64) (map[int64]*domainfeed.FeedStat, error)
}

func WithFeedStatSource(source FeedStatSource) FeedCacheOption {
	return func(cache *FeedCache) { cache.statSource = source }
}
...
func getStatsWithMode(ctx context.Context, client redisStatCacheClient, videoIDs []int64, sequential bool) (map[int64]*domainfeed.FeedStat, error) {
	...
	// A miss is filled by the application's batched PostgreSQL read. Retired
	// base/shard counters are never combined with a newer database snapshot.
...
func (c *FeedCache) SetStats(ctx context.Context, stats map[int64]*domainfeed.FeedStat, ttl time.Duration) error {
	return publishFeedStatSnapshots(ctx, c.client, c.statSource, stats, ttl)
}
...
func (c *FeedCache) SetVideoStat(ctx context.Context, stat *domaininteraction.VideoStat) error {
	...
	return c.SetStats(ctx, map[int64]*domainfeed.FeedStat{
		stat.VideoID: {VideoID: stat.VideoID, Revision: stat.Revision, LikeCount: stat.LikeCount,
			CommentCount: stat.CommentCount, FavoriteCount: stat.FavoriteCount},
	}, actionStatJSONTTL)
}
...
func (c *FeedCache) SetActionState(ctx context.Context, userID int64, videoID int64, actionType string, active bool, idempotencyKey string, initialStat *domaininteraction.VideoStat, initialState *domaininteraction.ActionStateSnapshot, mutation applicationinteraction.ActionMutation) (*applicationinteraction.ActionStateResult, error) {
	...
	// Provisional response values only. The service persists the action before
	// success and replaces these with an authoritative PostgreSQL snapshot.
	if initialStat != nil {
		result.LikeCount = initialStat.LikeCount
		result.FavoriteCount = initialStat.FavoriteCount
	}
	if actionType == domaininteraction.ActionTypeLike {
		result.LikeCount = clampRedisCount(result.LikeCount + result.Delta)
	} else {
		result.FavoriteCount = clampRedisCount(result.FavoriteCount + result.Delta)
	}
	return result, nil
}
...
func feedStatKey(videoID int64) string {
	return fmt.Sprintf("video:stat:v2:%d", videoID)
}
