...
func TestMultimodalMetricsUseOnlyClosedLabels(t *testing.T) {
	...
	if testutil.ToFloat64(provider)-before[0] != 1 || testutil.ToFloat64(unknownProvider)-before[1] != 1 ||
		testutil.ToFloat64(cache)-before[2] != 1 || testutil.ToFloat64(unknownCache)-before[3] != 1 ||
		testutil.ToFloat64(hybrid)-before[4] != 1 || testutil.ToFloat64(unknownHybrid)-before[5] != 1 {
		t.Fatal("multimodal labels were not folded to closed values")
	}
	for _, descriptor := range []string{
		provider.Desc().String(), MultimodalProjectionTotal.WithLabelValues("upserted").Desc().String(),
