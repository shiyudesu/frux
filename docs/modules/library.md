...
| 页面 | 接入能力 |
| --- | --- |
| 本人个人主页 | 喜欢、收藏、观看历史、稍后再看独立分页状态；个人页不再展示推荐 Tab |
| 内容库连续播放 | 点击喜欢、收藏、观看历史或稍后再看卡片后，以全屏覆盖层从所选项开始按列表顺序上下切换、连续播放和临近分页；关闭后保留原 Tab、列表和滚动位置 |
| 观看历史 | 按媒体位置显示已看秒数/已看完，兼容旧 `last_watch_ms`，支持单项删除和清空 |
| 稍后再看 | Feed/内容库播放“更多”菜单可幂等加入；稍后再看列表和连续播放可乐观移除，失败时只恢复目标项 |
