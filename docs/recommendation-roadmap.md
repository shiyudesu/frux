...
| `accept-session-semantic-active-rollout` | 已完成、待归档 | 通过显式 Session-only runtime 和正常 API 精确验收 active v3 Cohort，证明语义结果、Snapshot 复用、fallback、零模型调用与窄清理；最终 v3 disabled/not deleted，默认 runtime 恢复关闭 |
