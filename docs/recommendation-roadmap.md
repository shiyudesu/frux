...
| `add-session-semantic-acceptance-runner` | 已完成，待补足第三个真实向量 Fixture 后执行验收与归档 | 使用已有真实 active-contract 视频向量、临时低 Cohort 策略和正常行为/Feed API，验证真实 `semantic_session`、Request Log、Quota、Snapshot 二页复用与推荐阶段零模型调用；Runner 不上传、不生成向量 |
