...
| `add-session-semantic-recommendation` | 已完成并通过零模型调用技术验收，待归档 | 使用可信当前/近期会话事实组合已有 active-contract 视频向量，新增默认关闭的 `semantic_session` Exact Recall、`semantic_similarity`、Confidence、Quota Underfill 与脱敏证据；版本化 Golden Set 通过，未 Shadow、未 Rollout |
