...
| `add-session-semantic-recommendation` | 已提案，待实施 | 使用可信当前/近期会话事实组合已有 active-contract 视频向量，新增默认关闭的 `semantic_session` Exact Recall、`semantic_similarity`、Confidence、Quota Underfill 与脱敏证据；不训练、不做长期画像、不在推荐请求中调用模型 |
