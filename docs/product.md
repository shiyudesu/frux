...
| 推荐 | 已实现 | 已发布公开视频候选召回、曝光判断、曝光写入 |
| 互动 | 已实现 | 点赞、收藏、评论、删除评论、异步落库、作者获赞聚合 |
...
| 已实现 | 个人主页内容中心 | 一级 Tab 为作品、推荐、喜欢、收藏、观看历史、稍后再看；各列表按需加载并保留独立状态 |
...
个人主页只呈现已有真实领域能力。“短剧”和“我的预约（Appointments）”没有后端模型、接口或页面入口，明确不在当前产品范围内，也不渲染占位 Tab。收藏、观看历史、稍后再看和私密作品始终为本人能力；Web 不提供“公开收藏”控制或文案，当前仅喜欢列表可通过隐私设置公开。资料与喜欢隐私由一次原子保存提交。私密、下架和删除作品不仅从列表中裁剪，本地视频/封面还按不可变上传所有权和有效视频引用授权，旧 `/uploads` URL 不可绕过隐私。
